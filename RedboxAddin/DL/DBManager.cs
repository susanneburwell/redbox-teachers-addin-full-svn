...
namespace RedboxAddin.DL
{
    class DBManager
    {
        ...
        public List<RAvailability> GetAvailability(DateTime weekbegining)
        {
                ...
                if (msgDs != null)
                {
                    foreach (DataRow dr in msgDs.Tables[0].Rows)
                    {
                        RAvailability objAvail = new RAvailability()
                        {
                            ...
                            //Pay = dr["Pay"].ToString(),
                            //PofA = dr["PofA"].ToString(),
                            //QTS = dr["QTS"].ToString(),
                            //Wants = dr["Wants"].ToString(),
                            //YrGroup = dr["YrGroup"].ToString(),
        ...
        public string GetContactIDfromFullName(string fullName)
        {
            try
            {
        ...
        private string GetAvailabilitySQL(DateTime weekbegining)
        {
            ...
            string SQLstr = "Select FirstName+' '+Lastname as Teacher, s1.SchoolName as Monday, s2.SchoolName as Tuesday, s3.SchoolName as Wednesday, " +
                            "s4.SchoolName as Thursday, s5.SchoolName as Friday " +
                            "FROM [tblContacts] " +

                            "LEFT JOIN " +
                            "( " +
                            "SELECT [Schools].SchoolName, [MasterBookings].contactID " +
                            "FROM [MasterBookings] " +
                            "JOIN [Schools] " +
